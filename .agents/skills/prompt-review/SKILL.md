...
- 文字列 + 数値 → **プロジェクト名** + **日数フィルタ**（例: `sample-project 30`）
...
- 文字列のみ（例: `sample-project`） → `--project sample-project`
- 文字列 + 数値（例: `sample-project 30`） → `--project sample-project --days 30`
  ...
  "sources": [
    {
      ...
      "status": "検出",
      "messages": [
        {"text": "プロンプト本文", "timestamp": "2025-09-29 03:16", "project": "sample-project"}
      ],
      "period": "2025-09-29 03:16 〜 2026-03-12 04:58"
    }
  ],
  "project_stats": {
    "example-api": {"count": 668, "tools": ["Claude Code"]},
    "sample-project": {"count": 215, "tools": ["Claude Code"]}
  },
  "secret_warnings": [
    {
      "tool": "Claude Code",
      ...
      "type": "OpenAI API Key",
      "masked_value": "<masked-secret>",
      "prompt_excerpt": "APIキーは<redacted>..."
    }
  ]
}
```
