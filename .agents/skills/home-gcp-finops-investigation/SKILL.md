...
description: Google Cloud費用調査の補助ノート。認証、Billing Export、Recommender、CUD/SUDの解釈で詰まったときに、過去の調査で得た注意点を参照する。
---
...
調査全体を進めるスキルや手順がある場合は、それを主に使う。このノートは
認証や数値の解釈で必要になった項目だけ併読し、もう一度調査を始めない。
過去の仕様・料金の記述は、今回の判断へ使う前に現行の公式資料で確かめる。
...
- **権限は時間経過で変化する**: 別セッションや対象変更時は必要な API の可否を確認する。同じ対象・認証で得た今回の結果は再利用する
...
8. 数値と対象の検証。独立レビューを行った場合も、指摘を証拠と照合して妥当なものを反映
...
以下は継続調査での配置例。依頼先の形式・保存先があればそちらに合わせ、不要なファイルや外部向け文面を自動追加しない。
